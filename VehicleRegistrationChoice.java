...
public class VehicleRegistrationChoice extends JFrame implements ActionListener
...
public  JTextField  tf_UserName,tf_pass;
public  JButton btnSubmit,btnCancel;
public  ButtonGroup bg;
public  String show;
	
VehicleRegistrationChoice()
{
    //frame
	super(" VehicleRegistrationChoice");
	setLayout(null);
	setSize(1050,800);
	
	//Title Label
	lb_Title = new JLabel("WELCOME TO RTO VEHICLE REGISTRATION");
	lb_Title.setFont(new java.awt.Font("Times New Roman",3,18));
	lb_Title.setOpaque(true);
	lb_Title.setBackground(Color.RED);
	lb_Title.setBounds(140, 80, 390, 20);
	add(lb_Title);
	...
	btnExistingUserLogin = new JButton("Vehicle Registration");
	btnExistingUserLogin.setBounds(180, 250, 200, 40);
	add(btnExistingUserLogin);
	btnExistingUserLogin.addActionListener(this);
	btnExistingUserLogin.setActionCommand("Vehicle Registration");	//Add action listener to btnCancel
	...
	// TODO Auto-generated method stub
	  ...
	  String cmd = obj.getActionCommand();
		...
		/*	 if(cmd.equals("Submit"))
			...
			else if(cmd.equals("New User(Register First"))
			{
				new NewUserRegistration();
				this.dispose();
	     	}
			else if(cmd.equals("Vehicle Registration"))
			{
			    new VehicleRegistration();
				this.dispose();
	     	}
